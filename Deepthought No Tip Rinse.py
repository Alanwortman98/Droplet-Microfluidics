...
class Screen:
    ...
    def load_variables(cls):
        ...
        labels = ["PFD Dwell Time (s)" , "Organic Dwell Time (s)" , "Pause Time After Pump Stop (s)" ,
                  "Droplet Multiplicity" , "Motor Feedrate (mm/min)"]
        ...
        ttk.Button(input_window_load_vars , text="Upload" , command=get_inputs).grid(row=len(labels) , column=0 ,
                                                                                     columnspan=2 , pady=10
                                                                                     )

        cls.root.wait_window(input_window_load_vars)

        file_path_template_csv = filedialog.askopenfilename(title="Select a CSV File" ,
    ...
    def initialize(self):  # This takes the input .csv file and makes combinations with it
        ...
        try:
            ...
            word_fragments_to_remove = ['trace' , 'wash' , 'mark' , 'rinse' , 'pfd' , 'perfluorodecalin' ,
                                        'oil']  # Omits tracer, carrier fluid, and wash wells from being included in screen combos
            ...
            components_coords = [[w for w in wells_coords if df.loc[wells_coords.index(w) , 'class'] == c] for c in
                                 components]
            ...
            components_names = [[w for w in wells_names if df.loc[wells_names.index(w) , 'class'] == c] for c in
                                components]
                    ...
                    with open(gcode_file_path , 'w') as file:
                        for i in range(len(self.combinations)):  # for every combination
                            for j in range(self.repeats):  # range of repeats you want to make
                                sample_well(self.PFD_well_coordinates[0] , self.PFD_well_coordinates[1],
                                            self.PFD_dwell , self.pause_after_pump_stop , self.feedrate
                                            )  # Sample carrier fluid
                                sample_well(self.wash_well_coordinates[0] , self.wash_well_coordinates[1],
                                            self.PFD_dwell , self.pause_after_pump_stop , self.feedrate
                                            )  # Sample carrier fluid
                                sample_well(self.PFD_well_coordinates[0] , self.PFD_well_coordinates[1] ,
                                            self.PFD_dwell , self.pause_after_pump_stop , self.feedrate
                                            )  # Sample carrier fluid
                                # tip_rinse(self.wash_well_coordinates[0] , self.wash_well_coordinates[1] ,
                                #           self.pause_after_pump_stop , self.feedrate
                                #           )  # Tip rinse
                                # sample_well(self.PFD_well_coordinates[0] , self.PFD_well_coordinates[1] ,
                                #             self.PFD_dwell , self.pause_after_pump_stop , self.feedrate
                                #             )  # Sample carrier fluid
                                for k in range(len(self.combinations[0])
                                               ):  # Each well in a combination (combinations don't include carrier fluid) Note this was recently changed from 0 to 1
                                    sample_well(self.combinations[i][k][1][0] , self.combinations[i][k][1][1] ,
                                                self.organic_dwell , self.pause_after_pump_stop , self.feedrate
                                                )  # Sample stock solution
                                #     # tip_rinse(self.wash_well_coordinates[0] , self.wash_well_coordinates[1] ,
                                #     #           self.pause_after_pump_stop , self.feedrate
                                #     #           )  # Tip Rinse
                                # if (
                                #         i * self.repeats + j + 1) % 10 == 0:  # samples from tracer well after every 10th total combination.
                                #     sample_well(self.PFD_well_coordinates[0] , self.PFD_well_coordinates[1] ,
                                #                 self.PFD_dwell , self.pause_after_pump_stop , self.feedrate
                                #                 )  # Sample carrier fluid
                                #     # tip_rinse(self.wash_well_coordinates[0] , self.wash_well_coordinates[1] ,
                                ...
                                #     #             self.PFD_dwell , self.pause_after_pump_stop , self.feedrate
                                #     #             )  # Sample carrier fluid
                                #     sample_well(self.tracer_well_coordinates[0] , self.tracer_well_coordinates[1] ,
                                #                 self.organic_dwell , self.pause_after_pump_stop , self.feedrate
                                #                 )  # Sample tracer well
                                #     # tip_rinse(self.wash_well_coordinates[0] , self.wash_well_coordinates[1] ,
                                #     #           self.pause_after_pump_stop , self.feedrate
    ...
    def visualize(self):  # This writes a .csv file of all the combinations to keep track of what's in each droplet
        ...
        try:
            ...
            df_combinations = df_combinations.loc[df_combinations.index.repeat(self.repeats)].reset_index(drop=True
                                                                                                          )  # repeats the index a certain number of times to match the number of times you want to form a reaction
            df_combinations.index += 1
            column_names = df_combinations.columns.values
            df_combinations[column_names] = df_combinations.apply(lambda x: tuple([val[1] for val in x])
                                                                  )  # drops the well number associated with a reagent in each cell
            rxn_num = [a+1 for a in range(len(self.combinations)) for i in range(self.repeats)]
            df_combinations.insert(0, 'reaction #', rxn_num)
            droplet_num = [a + 1 for a in range(len(df_combinations.index))]
            df_combinations.insert(0 , 'droplet #' , droplet_num)
    ...
    def plate_summary(
            self):  # Note: this is going to give a weird output because empty wells are dropped in self.dataframe
        df = self.dataframe

        # Create a scatter plot with a single trace
        fig = go.Figure()

        blues = ['#edf5ff' , '#001141' , '#001d6c' , '#002d9c' , '#0043ce' , '#0f62fe' , '#4589ff' , '#78a9ff' ,
                 '#a6c8ff' , '#d0e2ff']
        purples = ['#edf5ff' , '#1c0f30' , '#31135e' , '#491d8b' , '#6929c4' , '#8a3ffc' , '#a56eff' , '#be95ff' ,
                   '#d4bbff' , '#e8daff' , '#f6f2ff']
        teals = ['#edf5ff' , '#081a1c' , '#022b30' , '#004144' , '#005d5d' , '#007d79' , '#009d9a' , '#08bdba' ,
                 '#3ddbd9' , '#9ef0f0' , '#d9fbfb']
...
my_screen=Screen.load_variables()
my_screen.initialize()
print(my_screen.combinations_names)
print(my_screen.combinations)
print(my_screen.repeats)
my_screen.visualize()
